...
def get_extract_label(art_sents, abs_sents):
    ...
    for abst in abs_sents: 
        ...
        rouges = list(map(
            compute_rouge_l(reference=abst, mode='f'),
                        art_sents))
        rouge1 = list(map(
            compute_rouge_n(reference=abst,n=1,mode='f'),
                        art_sents))
        rouge2 = list(map(
            compute_rouge_n(reference=abst,n=2,mode='f'),
                        art_sents))

        # ext = max(indices, key=lambda i: (rouges[i]))
        ext = max(indices, key=lambda i: (rouges[i]+rouge1[i]+rouge2[i])/3)
        indices.remove(ext)
