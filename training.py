...
class BasicPipeline(object):
    ...
    def train_step(self):
        ...
        # -------------------------------------------------------------
        # Extractor
        # fw_args: ([batch,sentence_nums],)
        # bw_args: ([batch,max_label_nums],)
        # net_out: [batch, max_label_nums, sentence_nums] 
        # backward and update ( and optional gradient monitoring )
        # loss_args: (2*[batch,max_label_nums, sentence_nums])
        # -------------------------------------------------------------
        # Abstractor
        # fw_args: 5*([batch,max_length],)
        #   [1] Article, [batch, max_length]
        #   [2] List of max_length,
        #   [3] Abstract, [batch, max_length]
        #   [4] Expand_Article,[batch, max_length]
        #   [5] Expand_vocab

        # bw_args: ([batch, max_length],)
        # -------------------------------------------------------------
        self._net.train()
...
class BasicTrainer(object):
    ...
    def __init__(self, pipeline, save_dir, ckpt_freq, patience,
        ...
        if os.path.exists(join(save_dir, 'ckpt')):
            pass
        else:    
    ...
    def log(self, log_dict):
        ...
        print('train step: {}, {}: {:.4f}, {}: {:.4f}, {}: {:.4f}\r'.format(
            self._step,
            'loss' if 'loss' in log_dict else 'reward',
            self._running_loss,
            'recall', log_dict['recall'] if 'recall' in log_dict else 0,
            'precision', log_dict['precision'] if 'precision' in log_dict else 0), end='\n')
