...
class RLDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        ext_sents = js_data['extracted']

        return art_sents, abs_sents, ext_sents
...
def train(args):
    ...
    # make net
    if os.path.isdir(os.path.join(args.path, 'ckpt')) and len(os.listdir(os.path.join(args.path, 'ckpt'))) > 0:
            agent,target_agent, agent_vocab, abstractor, net_args = load_rl_ckpt(args.abs_dir,args.path,args.cuda)
            use_ckpt = 1
    else:
        agent, target_agent, agent_vocab, abstractor, net_args = configure_net(
    ...
    if args.abs_dir is not None and not use_ckpt:
        dirs = args.abs_dir 
        abs_ckpt = {}
        abs_ckpt['state_dict'] = load_best_ckpt(dirs)
        abs_vocab = pkl.load(open(join(dirs, 'vocab.pkl'), 'rb'))
        abs_dir = join(args.path, 'abstractor')
        if os.path.exists(join(abs_dir, 'ckpt')):
            pass
        else:
    ...
    pipeline = A2CPipeline(meta['net'], agent, target_agent, abstractor,
...
if __name__ == '__main__':
    ...
    # model options
    parser.add_argument('--abs_dir', action='store',default='abstractor/model',
                        help='pretrained summarizer model root path')
    parser.add_argument('--ext_dir', action='store',default='extractor/model',
