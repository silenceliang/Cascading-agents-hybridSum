...
@curry
def token_nums(max_len, texts):
    return [len(t.split()[:max_len]) for t in texts]
...
def convert_batch_extract_ptr(unk, word2id, batch):
    def convert_one(sample):
        ...
        # source_sents = reconstruct_for_bert(source_sents)
        # tokenized_text = tokenizer.tokenize(source_sents)
        ...
        # id_sents = indexed_tokens
        # return id_sents, extracts, segments
...
def batchify_fn_extract_ptr(pad, data, cuda=True):
    ...
    tgt_nums = list(map(len, list(map(remove_last, targets))))
    """ 沒有remove 最後一個"""
    tar_in_retain = pad_batch_tensorize(targets, pad=-0, cuda=cuda)
    #  use 0 here for feeding first conv sentence repr.
    # tgt_nums = list(map(len, targets))
    """ tar_in ---> target  """
    fw_args = (sources, src_nums, tar_in, tar_in_retain, tgt_nums)
    loss_args = (target, )
    return fw_args, loss_args
